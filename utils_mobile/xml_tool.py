...
from utils_mobile.ocr_tools import XML_OCR_Matcher
import argparse
import traceback
import time
...
class UIXMLTree:
            ...
            if isinstance(value, dict):
                if "rotation" in value:
                    if not self.app_name:
						# assert self.package_name is not None, "package_name is None"
                        # app_name = f"The current APP's package name is {self.package_name}.\n"
                        # The current screenshot's description is shown:
                        app_name = ""
                        if check_version_size(self.xml_version, 'v4'):
                            if check_version_size(self.xml_version, 'v4.1'):
                                width, height = smart_resize_func(self.width, self.height, max_pixels=1000*1000)
                                relative_text = ""
                            else:
                                width = self.max_limit
                                height = self.max_limit
                                relative_text = "The value in bounds is relative to the screenshot's size."
                            app_name += f"The screenshot's size is {width}x{height}.{relative_text}\n"
                            app_name += f"The tree structure description of the current screenshot is shown:"
                        elif check_version_size(self.xml_version, 'v3'):
                            # app_name += f"The screenshot's size is {self.width}x{self.height}\n"
                            app_name += f"The tree structure description of the current screenshot is shown:"
                        else:
                            # app_name += f"The screenshot's size is {self.width}x{self.height}\n"
    ...
    def scale_bound_both_side(self, root, max_limit=999):
        ...
        for node in root.iter():
            if 'bounds' not in node.attrib and 'raw_bounds' not in node.attrib:
                continue
            bound = bounds_to_coords(node.attrib['raw_bounds'])
            x1, y1, x2, y2 = bound
            ...
            if scaled_bbox in map_for_scaling['scaled2raw'] and map_for_scaling['scaled2raw'][scaled_bbox] != node.attrib['raw_bounds']:
                exist_bound = bounds_to_coords(map_for_scaling['scaled2raw'][scaled_bbox])
                new_bound = bounds_to_coords(node.attrib['raw_bounds'])
                for a, b in zip(exist_bound, new_bound):
                    difference = abs(a - b)
                    if difference > 3:
                        warning = f"Warning: {scaled_bbox} already exists in map_for_scaling, original bound: {node.attrib['raw_bounds']}, existing bound: {map_for_scaling['scaled2raw'][scaled_bbox]}"
                        # print(warning)
            ...
            if scaled_bbox in map_for_scaling['scaled2raw']:
                if check_is_matchable(node):
                    if is_matchable_nodes[map_for_scaling['scaled2raw'][scaled_bbox]] and node.attrib['raw_bounds'] != map_for_scaling['scaled2raw'][scaled_bbox]:
            ...
            is_matchable_nodes[node.attrib['raw_bounds']] = check_is_matchable(node)
            map_for_scaling['scaled2raw'][scaled_bbox] = node.attrib['raw_bounds']
            map_for_scaling['raw2scaled'][node.attrib['raw_bounds']] = scaled_bbox
        return root, map_for_scaling

    def scale_bound_smart_resize(self, root, max_limit=999):
        assert self.height is not None and self.width is not None
        ...
        # Get scaled dimensions using smart_resize_func
        scaled_height, scaled_width = smart_resize_func(
            height=self.height,
            width=self.width,
        ...
        scale_factor_width = scaled_width / self.width
        scale_factor_height = scaled_height / self.height
        ...
        for node in root.iter():
            ...
            # Convert back to bounds string format
            scaled_bounds = coords_to_bounds([scaled_x1, scaled_y1, scaled_x2, scaled_y2])
            scaled_bbox = f"[{scaled_x1},{scaled_y1},{scaled_x2},{scaled_y2}]"
            ...
            # Store mapping between original and scaled bounds
            is_matchable_nodes[node.attrib['raw_bounds']] = check_is_matchable(node)
            map_for_scaling['scaled2raw'][scaled_bbox] = node.attrib['raw_bounds']
            map_for_scaling['raw2scaled'][node.attrib['raw_bounds']] = scaled_bbox
        return root, map_for_scaling
    ...
    def xml_sparse(self):
        ...
        for element in self.root.iter():
            ...
            if area > self.maxArea:
                ...
                self.package_name = element.attrib['package']
        ...
        if check_version_size(self.xml_version, 'v4'):
            if check_version_size(self.xml_version, 'v4.1'):
                scale_func = self.scale_bound_smart_resize
            else:
