...
if __name__ == '__main__':
    ...
    if args.parallel == 1:
        ...
        android_world_class.run_serial(suite)
        print_android_world_results(path = task_path)
    else:
        results = parallel_worker_android_world(class_, AndroidWorld_AutoTest, single_config.subdir_config(args.name), agent, args.parallel, suite)
        print(results)
