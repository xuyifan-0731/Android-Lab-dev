...
base_url = "http://172.18.192.104:10011"
...
def create_chat_completion(model, messages, temperature=0.8, max_tokens=2048, top_p=0.8, use_stream=False):
    ...
    response = requests.post(f"{base_url}/v1/chat/completions", json=data, stream=use_stream, proxies=proxy)
    print(response.text)
...
def simple_image_chat(use_stream=False, img_path=None):
    """
    ...
    create_chat_completion("Llama-3.2-11B-Vision", messages=messages, use_stream=use_stream)


if __name__ == "__main__":
    simple_image_chat(use_stream=False, img_path="/raid/xuyifan/digirl/logs/digirl-general-off2on/images/test1/1719900534.336901_0.png")