...
def build_android_prompt_gpt_androdi_gen_with_xml(
    instruction, last_actions, final_xml
) -> tuple[str, str]:
    system_msg = '''You are an expert in evaluating the performance of an android agent. The agent is designed to help a human user navigate on their device to complete a task. Given the user's intent, the agent's action history, the final state of the device, and the agent's response to the user, your goal is to list the conditions the task requires and their respective completion step (or -1 for not completed). There are two types of tasks:
...
'''
    prompt = f'''# User Intent: {instruction}
